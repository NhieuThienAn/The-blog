...
import { getUserStatistics, getPostStatistics } from '../../../api/api'; 
import Sidebar from '../Sidebar/Sidebar';
import { Card, Col, Row, Typography, Alert, Button, Table } from 'antd';
...
const StatisticsManagement = () => {
    ...
    const exportToExcel = () => {
        ...
        const colWidths = data[0].map(() => ({ wpx: 0 })); 
        data.forEach(row => {
            row.forEach((cell, i) => {
                const cellLength = String(cell).length * 10; 
                if (cellLength > colWidths[i].wpx) {
                    colWidths[i].wpx = cellLength; 
                }
            });
        });
